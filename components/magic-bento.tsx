...
const DEFAULT_GLOW_COLOR = "0, 173, 239"
...
function ParticleCard({
  ...
  return (
    <div
      ref={cardRef}
      className={`${className} particle-container magic-bento-card`}
...
function GlobalSpotlight({
  ...
  }, [gridRef, sectionRef, disableAnimations, enabled, spotlightRadius, glowColor])
...
function CardInner({ card }: { card: MagicBentoCard }) {
  const TitleTag = card.featured ? "h2" : "h3"
...
interface MagicBentoInnerProps extends MagicBentoProps {}
...
export function MagicBento({
  ...
  const cardClassName = (featured?: boolean) =>
    [
      ...
      featured ? "magic-bento-card--featured" : "",
    ]
      .filter(Boolean)
      .join(" ")
  ...
  return (
    <>
      ...
      <ol className="card-grid bento-section m-0 list-none p-0" ref={gridRef}>
        {cards.map((card, index) => {
          const className = cardClassName(card.featured)
          const props = {
            className,
            style: cardStyle,
          }
...
function StaticInteractiveCard({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      if (!clickEffect || !glowColor) return
      const rect = el.getBoundingClientRect()
      const x = e.clientX - rect.left
      const y = e.clientY - rect.top
      const maxDistance = Math.max(
