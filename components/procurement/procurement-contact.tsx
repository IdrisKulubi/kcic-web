import { procurementContact } from "@/lib/data/procurement"

export function ProcurementContact() {
  return (
    <section
      aria-labelledby="procurement-contact-title"
    ...
    >
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
        <div>
          ...
          >
            {procurementContact.heading}
          </h2>
          <p className="mt-2 m-0 max-w-[42ch] text-sm leading-6 text-[#cfdfcb]">
            {procurementContact.body}
          </p>
          <p className="mt-3 m-0 text-sm text-[#bdd0b9]">
            <a
              href={`mailto:${procurementContact.email}`}
              className="text-[#c9dcc4] underline-offset-4 hover:underline"
            >
              {procurementContact.email}
            </a>
            <span aria-hidden="true" className="mx-2">·</span>
            <a
              href={procurementContact.phoneHref}
              className="text-[#c9dcc4] underline-offset-4 hover:underline"
            >
              {procurementContact.phone}
            </a>
          </p>
        </div>
        <a
